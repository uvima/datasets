...
En este cuaderno vamos a analizar el dataset llamado [*ipc_series.xlsx*](../../../../files/ipc_series.xlsx). Este dataset presenta el [IPC Español](https://www.ine.es/dyngs/INEbase/es/operacion.htm?c=Estadistica_C&cid=1254736176802&menu=ultiDatos&idp=1254735976607) desde el año 2002 hasta el 2022 con una periodicidad mensual. El objetivo será analizar dicha serie temporal mediante un modelo Holt Winters.
...
# IPC
serie_ipc <- ts(rev(data$IPC), start = 2002, frequency = 12)

# creamos train/test partición
data.train <- window(serie_ipc, end = c(2022, 5))
data.test <- window(serie_ipc, start = c(2022, 6))
```
...
plot(data.train, main = "IPC Mensual desde 2002", xaxt = "n")
axis(1, at = seq(2002, 2022, 2)) # X-axis mostrar cada 2 años
...
-   **Parámetros de Suavizado**:
    -   **alpha**: El parámetro de suavizado para la nivel base es aproximadamente 0.920. Indica cuánto peso se le da a las observaciones más recientes para calcular el nivel base.
    -   **beta**: El parámetro de suavizado para la tendencia es 0, lo que sugiere que no se está considerando el suavizado para la tendencia en este modelo.
...
-   **Coeficientes**:
    -   **a**: El nivel base es aproximadamente 107. Representa el valor promedio de la serie temporal.
    -   **b**: La pendiente de la tendencia es alrededor de 0.176. Indica la tasa de cambio en la serie temporal.
    -   **s1 a s12**: Estos coeficientes representan los efectos estacionales para cada uno de los 12 meses del año. Los valores negativos indican una contribución negativa a la estacionalidad, mientras que los valores positivos indican una contribución positiva. Por ejemplo, los coeficientes positivos para s5, s6 y s7 sugieren un aumento estacional durante esos meses.
...
-   **Gráfico Autocorrelación de Residuos**: muestra la autocorrelación de los residuos a diferentes rezagos. Se espera que los residuos no estén correlacionados entre sí, lo que se reflejaría en que los puntos estén dentro de las bandas de confianza. Sin embargo hay varios fuera, lo que puede dejar evidencia de que no es un ajuste perfecto.
-   **Histograma de Residuos**: Muestra la distribución de los residuos. Si los residuos se distribuyen normalmente alrededor de cero, el histograma debería parecerse a una distribución normal. (parece que si, aunque con una cola larga a la derecha.)

**Predicción** Adicionalmente, se va a llevar a cabo una predicción sobre las observaciones sustraídas y se va a comparar con los valores reales a ver que efectividad tiene. Concretamente se va a hacer para los 7 meses posteriores. Comentar que cuanto más grande sea el test data, peor serán las métricas de la predicción, puesto que es muy difícil predecir en un periodo alejado del tiempo.
...
data.f1 <- forecast(mod1, h = 7)

# check accuracy
accuracy(data.f1, data.test)
```

En el conjunto de datos de test vemos que el MPE y el MAPE muestran alrededor de un 1.5% lo cual es buen indicador. Podemos repetir el estudio con ARIMA para ver si obtenemos cosas coherentes, ya que hemos visto que algunas hipótesis de los residuos no se cumplían en este modelo.
